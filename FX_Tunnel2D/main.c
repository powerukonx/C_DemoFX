/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Tunnel2D.
============================================================================= */
...
#define FX_NAME                 "FX_Tunnel2D by UKONX"
...
static bool bFxSetup (void)
{
  ...
  if (NULL != g_pSDL_Image)
  {
    ...
    for (int16_t l_s16PosY = (int16_t)(-(YMAX>>1u) ); l_s16PosY < (int16_t)(YMAX>>1u); l_s16PosY++)
    {
      for (int16_t l_s16PosX = (int16_t)(-(SCREEN_WIDTH>>1u) ); l_s16PosX < (int16_t)(SCREEN_WIDTH>>1u); l_s16PosX++)
      {
        uint8_t l_u8Angle     = 0u;
        uint8_t l_u8Distance  = 0u;

        if (   (0 != l_s16PosX)
            || (0 != l_s16PosY) )
        {
          l_u8Distance = (uint8_t)(l_fRatio * TEXTURE_HEIGHT / sqrt( (l_s16PosX * l_s16PosX) + (l_s16PosY * l_s16PosY ) ) ) % TEXTURE_HEIGHT;
          l_u8Angle    = (uint8_t)(0.5f * TEXTURE_WIDTH * atan2(l_s16PosY , l_s16PosX) / M_PI);
        }
        else
        {
          l_u8Angle    = 0u;
        ...
        g_au8DistanceLUT[l_s16PosX + (SCREEN_WIDTH>>1u)][l_s16PosY + (YMAX>>1u)]  = l_u8Distance;
        g_au8AngleLUT[l_s16PosX + (SCREEN_WIDTH>>1u)][l_s16PosY + (YMAX>>1u)]     = l_u8Angle;
      }
    }
...
static bool bFxLoop (void)
{
  ...
  /* -User specific- */
  static uint16_t l_u8ShiftX = 0;
  static uint16_t l_u8ShiftY = 0;
