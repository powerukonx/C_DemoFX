/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Plasma2.
============================================================================= */
...
#define FX_NAME                 "FX_Plasma2 by UKONX"
...
static void vCreatePalette      (uint8_t p_u8Shift);
...
Describe    :   Dynamic plasma palette.
Parameters  :   None.
Returns     :   None.
==============================================================================*/
static void vCreatePalette (uint8_t p_u8Shift)
{
  ...
  {
    uint8_t l_u8Index           = 256.0f * M_PI * 2.0f * ( (p_u8Shift + l_u16Index) % TABLE_SIZE) / 256.0f;
    uint8_t l_u8Red             = g_pau8Sinus[l_u8Index];
    uint8_t l_u8Green           = g_pau8Sinus[(3 * l_u8Index) % TABLE_SIZE];
    uint8_t l_u8Blue            = g_pau8Sinus[(5 * l_u8Index) % TABLE_SIZE];
...
{
  /* Locals variables declaration. */
  static uint8_t l_u8Shift = 0u;

  /* Make palette. */
  vCreatePalette (l_u8Shift);
  ...
  for (uint16_t l_u16IndexY = 0u; l_u16IndexY < YMAX; l_u16IndexY++)
  {
    for (uint16_t l_u16IndexX = 0u; l_u16IndexX < SCREEN_WIDTH; l_u16IndexX++)
    {
      uint8_t l_u8C     = 32 * M_PI * 2;
      uint8_t l_u8Val1  = g_pau8Sinus[(uint8_t)(dist((SCREEN_WIDTH>>1u) - l_u16IndexX, (YMAX>>1u) - l_u16IndexY) * 2)];
      uint8_t l_u8Val2  = g_pau8Sinus[(uint8_t)(l_u8C * l_u16IndexX / SCREEN_WIDTH)];
      uint8_t l_u8Val3  = g_pau8Sinus[(uint8_t)(l_u8C * l_u16IndexY / YMAX)];
      g_pau32Buffer[(l_u16IndexY * SCREEN_WIDTH) + l_u16IndexX] = g_pau32Palette[(l_u8Val1 + l_u8Val2 + l_u8Val3) >> 3u];
    }
  }
