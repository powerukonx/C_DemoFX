/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Fire.
============================================================================= */
...
#define FX_NAME                 "FX_Fire by UKONX"
...
#define PALETTE_SIZE      ((uint16_t)256)
/* -User specific- */
...
/* -User specific- */
static void vCreatePalette      (void);
static void vMakeRandomPixels   (void);
static void vMakeFire           (void);
...
static bool bFxSetup (void)
{
  ...
  /* -User specific- */
  g_pau32Palette = (uint32_t *)malloc (sizeof (uint32_t) * PALETTE_SIZE);
...
static bool bFxLoop (void)
{
  ...
  /* Display FX. */
  for (uint16_t l_u16IndexY = 0u; l_u16IndexY < YMAX - (RANDOM_LINE_SIZE<<2u); l_u16IndexY++)
  {
    for (uint16_t l_u16IndexX = 0u;l_u16IndexX < SCREEN_WIDTH; l_u16IndexX++)
    {
      Uint32 *p_ecran =  g_pSDL_Screen->pixels
                       + ( (l_u16IndexY + (SCREEN_HIGH / 6) + 32u) * SCREEN_WIDTH * g_pSDL_Screen->format->BytesPerPixel)
...
/* -User specific- */
/*==============================================================================
Function    :   vCreatePalette
Describe    :   Create fire palette.
Parameters  :   None.
Returns     :   None.
==============================================================================*/
static void vCreatePalette (void)
{
  /* Make palette. */
  for (uint8_t l_u8Index = 0u; l_u8Index < 80u; l_u8Index++)
  {
    uint8_t l_u8Red   = 0u;
    uint8_t l_u8Green = 0u;
    uint8_t l_u8Blue  = 0u;
    g_pau32Palette[l_u8Index] = MAKE_RGB(l_u8Red, l_u8Green, l_u8Blue);
  }

  for (uint8_t l_u8Index = 80u; l_u8Index < 112u; l_u8Index++)
  {
    uint8_t l_u8Red   = (l_u8Index - 80u)*8u;
    uint8_t l_u8Green = 0u;
    uint8_t l_u8Blue  = 0u;
    g_pau32Palette[l_u8Index] = MAKE_RGB(l_u8Red, l_u8Green, l_u8Blue);
  }

  for (uint8_t l_u8Index = 112u; l_u8Index < 128u; l_u8Index++)
  {
    uint8_t l_u8Red   = 255u;
    uint8_t l_u8Green = (l_u8Index - 112u) * 16u;
    uint8_t l_u8Blue  = 0u;
    g_pau32Palette[l_u8Index] = MAKE_RGB(l_u8Red, l_u8Green, l_u8Blue);
  }

  for (uint8_t l_u8Index = 128u; l_u8Index != 0u; l_u8Index++)
  {
    uint8_t l_u8Red   = 255u;
    uint8_t l_u8Green = 255u;
    uint8_t l_u8Blue  = (l_u8Index - 128u)*2;
    g_pau32Palette[l_u8Index] = MAKE_RGB(l_u8Red, l_u8Green, l_u8Blue);
  }
}
...
/*==============================================================================
Function    :   vMakeRandomLine.
Describe    :   Make random pixels.
Parameters  :   None.
Returns     :   None.
==============================================================================*/
static void vMakeRandomPixels (void)
{
  /* Locals variables declaration. */
  static uint32_t l_u32NoiseShiftRegister = 0x24000;

  /* Make random pixels. */
  for (uint16_t l_u16IndexY = (YMAX - RANDOM_LINE_SIZE); l_u16IndexY < YMAX; l_u16IndexY++)
  {
    for (uint16_t l_u16IndexX = 0u; l_u16IndexX < SCREEN_WIDTH; l_u16IndexX++)
    {
      uint8_t l_u8Noise = ((((l_u32NoiseShiftRegister & BIT2)>>2) ^ (l_u32NoiseShiftRegister & BIT0)) & BIT0);
      l_u32NoiseShiftRegister  = ((l_u32NoiseShiftRegister >> 1) & MASK_16BITS) + (l_u8Noise << 16);

      if (l_u32NoiseShiftRegister & BIT1)
      {
        g_pau8Buffer[(l_u16IndexY * SCREEN_WIDTH) + l_u16IndexX] = rand()%1000;
      }
      else
      {
        g_pau8Buffer[(l_u16IndexY * SCREEN_WIDTH) + l_u16IndexX] = rand()%512;
      }
    }
  }
}
...
{
  /* make fire. */
  for (uint16_t l_u16IndexY = (RANDOM_LINE_SIZE>>1u); l_u16IndexY < (YMAX - 1); l_u16IndexY++)
  {
    for (uint16_t l_u16IndexX = 1; l_u16IndexX < (SCREEN_WIDTH - 1); l_u16IndexX++)
    {
      uint16_t l_u16Sum =   g_pau8Buffer[l_u16IndexY * SCREEN_WIDTH + l_u16IndexX]
                          + g_pau8Buffer[l_u16IndexY * SCREEN_WIDTH + l_u16IndexX + 1]
                          + g_pau8Buffer[l_u16IndexY * SCREEN_WIDTH + l_u16IndexX - 1]
                          + g_pau8Buffer[(l_u16IndexY + 1) * SCREEN_WIDTH + l_u16IndexX]
                          + g_pau8Buffer[(l_u16IndexY + 1) * SCREEN_WIDTH + l_u16IndexX + 1]
                          + g_pau8Buffer[(l_u16IndexY + 1) * SCREEN_WIDTH + l_u16IndexX - 1]
                          + g_pau8Buffer[(l_u16IndexY - 1) * SCREEN_WIDTH + l_u16IndexX - 1]
                          + g_pau8Buffer[(l_u16IndexY - 1) * SCREEN_WIDTH + l_u16IndexX + 1];
       g_pau8Buffer[(l_u16IndexY - (RANDOM_LINE_SIZE>>1u)) * SCREEN_WIDTH + l_u16IndexX] = l_u16Sum/8.011f;
    }
  }
   ...
   for (uint16_t l_u16IndexY = 0U; l_u16IndexY < YMAX; l_u16IndexY++)
   {
     ...
     {
       g_pau32Buffer[(l_u16IndexY * SCREEN_WIDTH) + l_u16IndexX] = g_pau32Palette[g_pau8Buffer[(l_u16IndexY * SCREEN_WIDTH) + l_u16IndexX]];
     }
   }
}
