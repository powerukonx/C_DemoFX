...
static sMetaball_t *g_pasMetaball                               = NULL;
/* -User specific- */
...
static float fGetDist           (int16_t, int16_t, int16_t, int16_t);
static void vMakeMetaball       (void);
  ...
  if (NULL != g_pau8Sinus)
  {
    ...
    if (NULL != g_pau32Palette)
    {
      ...
      if (NULL != g_pau32Buffer)
      {
        ...
        if (NULL != g_pau32Mini)
        {
          g_pasMetaball = (sMetaball_t *)malloc (sizeof (sMetaball_t) * METABALL_NB);
          if (NULL != g_pasMetaball)
          {
            vInitializeFx ();
            l_bReturn = true;
          }
...
static void vInitializeFx (void)
{
  for (uint8_t l_u8Idx = 0u; l_u8Idx < METABALL_NB; l_u8Idx++)
  {
    g_pasMetaball[l_u8Idx].s16PosX = rand()%SCREEN_WIDTH;
    g_pasMetaball[l_u8Idx].s16PosY = rand()%YMAX;
    g_pasMetaball[l_u8Idx].s16MoveX = rand()%20;
    g_pasMetaball[l_u8Idx].s16MoveY = rand()%20;
    g_pasMetaball[l_u8Idx].u16Radius = 1800;
  }
}
...
/*==============================================================================
Function    :   fGetDist
Describe    :   Compute distance between two screen points.
Parameters  :   p_s16Pt1X = First point X position,
                p_s16Pt1Y = First point Y position,
                p_s16Pt2X = Second point X position,
                p_s16Pt2Y = Second point Y position.
Returns     :   Distance between two points.
==============================================================================*/
static float fGetDist (int16_t p_s16Pt1X, int16_t p_s16Pt1Y, int16_t p_s16Pt2X, int16_t p_s16Pt2Y)
{
  /* Compute diff. */
  int16_t l_s16DiffX = p_s16Pt1X - p_s16Pt2X;
  int16_t l_s16DiffY = p_s16Pt1Y - p_s16Pt2Y;

  /* Return distance. */
  return (sqrt (l_s16DiffX*l_s16DiffX + l_s16DiffY*l_s16DiffY) + 0.0001f);
}
...
{
  /* make effect. */
  for (int16_t l_s16IndexY = 0; l_s16IndexY < YMAX; l_s16IndexY++)
  {
    for (int16_t l_s16IndexX = 0; l_s16IndexX < SCREEN_WIDTH; l_s16IndexX++)
    {
      /* Compute sum of metaballs for this point. */
      float l_fSum = 0.0f;
      for (uint8_t l_u8IndexMetaball = 0u; l_u8IndexMetaball < METABALL_NB; l_u8IndexMetaball++)
      {
        l_fSum += g_pasMetaball[0].u16Radius / fGetDist (l_s16IndexX, l_s16IndexY, g_pasMetaball[l_u8IndexMetaball].s16PosX, g_pasMetaball[l_u8IndexMetaball].s16PosY);
      }

      /* Set color. */
      int32_t l_s32Color = (int32_t)l_fSum;
      if (l_s32Color > 255) l_s32Color = 255;
      if (l_s32Color < 0) l_s32Color = 0;
      g_pau32Buffer[(l_s16IndexY * SCREEN_WIDTH) + l_s16IndexX] = (l_s32Color<<16u) | (l_s32Color<<8u) | l_s32Color;
    }
  }
  ...
  for (uint8_t l_u8Idx = 0u; l_u8Idx < METABALL_NB; l_u8Idx++)
  {
    g_pasMetaball[l_u8Idx].s16PosX += g_pasMetaball[l_u8Idx].s16MoveX;
    g_pasMetaball[l_u8Idx].s16PosY += g_pasMetaball[l_u8Idx].s16MoveY;

    if (    (g_pasMetaball[l_u8Idx].s16PosX >= SCREEN_WIDTH)
        || (g_pasMetaball[l_u8Idx].s16PosX <= 0))
    {
      g_pasMetaball[l_u8Idx].s16MoveX *= -1;
      g_pasMetaball[l_u8Idx].s16PosX += g_pasMetaball[l_u8Idx].s16MoveX;
    }

    if (    (g_pasMetaball[l_u8Idx].s16PosY >= YMAX)
        || (g_pasMetaball[l_u8Idx].s16PosY <= 0))
    {
      g_pasMetaball[l_u8Idx].s16MoveY *= -1;
      g_pasMetaball[l_u8Idx].s16PosY += g_pasMetaball[l_u8Idx].s16MoveY;
    }
