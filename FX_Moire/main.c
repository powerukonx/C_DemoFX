/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Moire.
============================================================================= */
...
#define FX_NAME                 "FX_Moire by UKONX"
#define SCREEN_WIDTH            ((uint16_t)512)
#define SCREEN_HIGH             ((uint16_t)384)
...
#define TEXTURE_FILENAME        "./texture/sprite.png"
/* -User specific- */
...
static void vMakeMoire (void)
{
  ...
  /* Sprite 1 moving. */
  int16_t l_u8DepX = -(SCREEN_WIDTH>>1) + (SCREEN_WIDTH>>1)* sin(l_fAngle);
  int16_t l_u8DepY = -4*(YMAX>>1) + (YMAX>>1) * cos(l_fAngle);

  /* Display sprite 1. */
  for (int16_t l_s16SpriteY = -(g_pSDL_Image->h>>1); l_s16SpriteY < (g_pSDL_Image->h>>1); l_s16SpriteY++)
  {
    for (int16_t l_s16SpriteX = -(g_pSDL_Image->w>>1); l_s16SpriteX < (g_pSDL_Image->w>>1); l_s16SpriteX++)
    {
      uint16_t l_u16SpriteY = (uint16_t)(l_s16SpriteY + (g_pSDL_Image->h>>1) );
      uint16_t l_u16SpriteX = (uint16_t)(l_s16SpriteX + (g_pSDL_Image->w>>1) );

      uint8_t *p_texture =  (uint8_t *)g_pSDL_Image->pixels
                          + (l_u16SpriteY % g_pSDL_Image->h) * g_pSDL_Image->pitch
                          + (l_u16SpriteX % g_pSDL_Image->w) * g_pSDL_Image->format->BytesPerPixel;
      l_u16SpriteY += l_u8DepY;
      l_u16SpriteX += l_u8DepX;

      if (   (l_u16SpriteY >= 0)
          && (l_u16SpriteY < YMAX)
          && (l_u16SpriteX >= 0)
          && (l_u16SpriteX < SCREEN_WIDTH) )
      {
        g_pau32Buffer[(l_u16SpriteY * SCREEN_WIDTH) + l_u16SpriteX] = MAKE_RGB(*(p_texture + 0u), *(p_texture + 1u), *(p_texture + 2u) );
      }
    }
  }

  /* Sprite 2 moving. */
  l_u8DepX = -(SCREEN_WIDTH>>1) + (SCREEN_WIDTH>>1)* sin(l_fAngle*2);
  l_u8DepY = -3*(YMAX>>1) + (YMAX>>1) * cos(-l_fAngle*3);
