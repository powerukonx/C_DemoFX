/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Deform2.
============================================================================= */
...
#define FX_NAME                 "FX_Deform2 by UKONX"
...
static uint8_t  *g_pau8Cosinus                                  = NULL;
...
static bool bFxSetup (void)
{
  ...
  if (NULL != g_pau8Sinus)
  {
    g_pau8Cosinus = (uint8_t *)malloc (sizeof (uint8_t) * TABLE_SIZE);
    if (NULL != g_pau8Cosinus)
    {
      ...
      if (NULL != g_pau8Buffer)
      {
        ...
        if (NULL != g_pau32Buffer)
        {
          ...
          if (NULL != g_pau32Mini)
          {
            ...
            if (NULL != g_pSDL_Image)
            {
              /* Compute sinus/cosinus LUT. */
              for (uint16_t l_u16Angle = 0u; l_u16Angle < 256u; l_u16Angle++)
              {
                g_pau8Sinus[l_u16Angle]   = (uint8_t)(128 + 127*sin (l_u16Angle * 2.0f * M_PI/256.0f) );
                g_pau8Cosinus[l_u16Angle] = (uint8_t)(128 + 127*cos (l_u16Angle * 2.0f * M_PI/256.0f) );
              }
...
static bool bFxLoop (void)
{
  ...
  /* -User specific- */
  uint16_t l_u16IndexX;
  uint16_t l_u16IndexY;

  vMakeDeform ();
  vMakeAndDisplayMini ();

  /* Display FX. */
  for (l_u16IndexY = 0u; l_u16IndexY < YMAX - (RANDOM_LINE_SIZE<<2u); l_u16IndexY++)
  {
    for (l_u16IndexX = 0u; l_u16IndexX<SCREEN_WIDTH; l_u16IndexX++)
    {
      uint32_t *p_ecran =  g_pSDL_Screen->pixels
                         + ( (l_u16IndexY + (SCREEN_HIGH / 6u) + 32u) * SCREEN_WIDTH * g_pSDL_Screen->format->BytesPerPixel)
...
/* -User specific- */
/*==============================================================================
Function    :   vMakeDeform.
Describe    :   Make some defomrmation.
Parameters  :   None.
Returns     :   None.
==============================================================================*/
static void vMakeDeform (void)
{
  /* Locals variables declaration. */
  static uint32_t l_u32Angle0 = 0ul;
  static uint32_t l_u32Angle1 = 0ul;
  static uint32_t l_u32Angle2 = 0ul;
  static uint32_t l_u32depmin = 0ul;
  static uint32_t l_u32deplx  = 0ul;
  static uint32_t l_u32deply  = 0ul;

  /* Applique la deformation */
  for (uint16_t y = 0u; y < YMAX; y++)
  {
    for (uint16_t x = 0u; x < SCREEN_WIDTH; x++)
    {
      uint16_t y_1 = ( ( (g_pau8Sinus[(y + l_u32Angle1) % TABLE_SIZE] * g_pau8Sinus[(y + l_u32deply) % TABLE_SIZE] * g_pau8Sinus[(x + l_u32Angle0) % TABLE_SIZE]) >> 16u) ) % YMAX;
      uint16_t x_1 = ( ( (g_pau8Cosinus[(x - y + l_u32deplx + l_u32Angle2) % TABLE_SIZE] * g_pau8Sinus[(x + y + l_u32Angle0) % TABLE_SIZE]) >> 8u) ) % SCREEN_WIDTH;

      uint8_t *p_texture =  (uint8_t *)g_pSDL_Image->pixels
                          + ( (y_1 + l_u32deply) % g_pSDL_Image->h) * g_pSDL_Image->pitch
                          + ( (x_1 + l_u32deplx) % g_pSDL_Image->w) * g_pSDL_Image->format->BytesPerPixel;
      g_pau32Buffer[(y * SCREEN_WIDTH) + x] = MAKE_RGB(*(p_texture + 0u), *(p_texture + 1u), *(p_texture + 2u) );

   //   g_pau32Buffer[y * SCREEN_WIDTH + x] = u32GetPixel (g_pSDL_Image, ( (x_1 + l_u32deplx) % g_pSDL_Image->w), (y_1 + l_u32deply) % g_pSDL_Image->h);
    }
  }

  l_u32depmin++;

  l_u32deplx = (uint32_t)( (SCREEN_WIDTH>>1u) + (SCREEN_WIDTH>>1u) * sin (l_u32depmin * 2 * 3.1415f/720.0f) );
  l_u32deply = (uint32_t)( (YMAX>>1u) + (YMAX>>1u) * cos (l_u32depmin * 2 * 3.1415f/720.0f) );
