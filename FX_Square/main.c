/* =============================================================================
  ...
  Date:         23 07 2017
  Author:       Power.
  Description:  Oldschool demoeffect - FX Square.
============================================================================= */
...
#define FX_NAME                 "FX_Square by UKONX"
...
static const  char      sznamefile[]                            = {"./texture/FX_Square.png"};
static        uint8_t   g_u8PlasmaAngle1                        = 0u;
static        uint8_t   g_u8PlasmaAngle2                        = 0u;
/* -User specific- */
...
static bool bFxLoop (void)
{
  ...
  {
    g_pau32Buffer[((0)*SCREEN_WIDTH)+l_u16IndexX] = 0xAAAAAA;
    g_pau32Buffer[((1)*SCREEN_WIDTH)+l_u16IndexX] = 0xFFFFFF;
    g_pau32Buffer[((2)*SCREEN_WIDTH)+l_u16IndexX] = 0xAAAAAA;

    g_pau32Buffer[((YMAX-1)*SCREEN_WIDTH)+l_u16IndexX] = 0xAAAAAA;
    g_pau32Buffer[((YMAX-2)*SCREEN_WIDTH)+l_u16IndexX] = 0xFFFFFF;
    g_pau32Buffer[((YMAX-3)*SCREEN_WIDTH)+l_u16IndexX] = 0xAAAAAA;
  }
...
Returns     :   None.
==============================================================================*/
static void vFxDispSquare (uint16_t p_u16PosX, uint16_t p_u16PosY, uint8_t p_u8SquareNb)
{
  for (uint16_t l_u16IndexY = 0u; l_u16IndexY < 64; l_u16IndexY++)
  {
    for (uint16_t l_u16IndexX = 0u; l_u16IndexX < 64; l_u16IndexX++)
    {
      uint16_t l_u16TexX = (p_u8SquareNb / 4) * 64 + l_u16IndexX;
      uint16_t l_u16TexY = (p_u8SquareNb % 4) * 64 + l_u16IndexY;

      uint8_t *p_texture = (uint8_t *)g_pSDL_Image->pixels
                        +  (l_u16TexX  % g_pSDL_Image->w) * g_pSDL_Image->pitch
                        +  (l_u16TexY  % g_pSDL_Image->h) * g_pSDL_Image->format->BytesPerPixel;

    /*  if ( (l_u16IndexY == 0) || (l_u16IndexX == 0) || (l_u16IndexY == 63) || (l_u16IndexX == 63) )
      {
        uint16_t l_u16ScreenX =  l_u16IndexX + p_u16PosX;
        uint16_t l_u16ScreenY =  l_u16IndexY + p_u16PosY;

        if ((l_u16ScreenX < SCREEN_WIDTH) && (l_u16ScreenY < YMAX))
        {
          g_pau32Buffer[(l_u16ScreenY * SCREEN_WIDTH) + l_u16ScreenX] = 0xFFFFFF;
        }
      }
      else*/
      {
        ...
        uint32_t l_u32PixelColor = MAKE_RGB(*(p_texture + 0u), *(p_texture + 1u), *(p_texture + 2u) );

        if ((l_u16ScreenX < SCREEN_WIDTH) && (l_u16ScreenY < YMAX))
        {
          if (l_u32PixelColor == 0x000000)
          {
            uint8_t l_u8PlasmaVal =  g_pau8Sinus[(l_u16ScreenY + g_u8PlasmaAngle1) % TABLE_SIZE]
                                   + g_pau8Sinus[(l_u16ScreenX + g_u8PlasmaAngle2) % TABLE_SIZE]
                                   + g_pau8Sinus[(g_u8PlasmaAngle2 + (TABLE_SIZE>>1u) ) % TABLE_SIZE]
                                   + g_pau8Sinus[( (l_u16ScreenX>>1u) + (l_u16ScreenY>>1u) + g_u8PlasmaAngle1) % TABLE_SIZE];

            g_pau32Buffer[(l_u16ScreenY * SCREEN_WIDTH) + l_u16ScreenX] = g_pau32Palette[l_u8PlasmaVal];
          }
          else
          {
             g_pau32Buffer[(l_u16ScreenY * SCREEN_WIDTH) + l_u16ScreenX] = l_u32PixelColor;
          }
        }
      }
    }
...
{
  /* Locals variables declaration. */
  static float f_Angle1 = 0.0f;
  static float f_Angle2 = 0.0f;

  static uint16_t l_u16ScrollX = 0u;
  static uint16_t l_u16ScrollY = 0u;


  /* Display fx. */
  for (int16_t l_u16IndexY = 0; l_u16IndexY < YMAX; l_u16IndexY += 64)
  {
    for (int16_t l_u16IndexX = 0; l_u16IndexX < SCREEN_WIDTH; l_u16IndexX += 64)
    {

      uint16_t l_u16PosX = (l_u16IndexX + l_u16ScrollX) % SCREEN_WIDTH;
      uint16_t l_u16PosY = (l_u16IndexY + l_u16ScrollY) % YMAX;

      uint8_t l_u8SquareNbX = 4 + 4*sin (f_Angle1 + (l_u16PosX * 2.0f * 3.1415f/360.0f));
      uint8_t l_u8SquareNbY = 4 + 4*cos (f_Angle2 + (l_u16PosY * 2.0f * 3.1415f/360.0f));

      vFxDispSquare (l_u16PosX, l_u16PosY, l_u8SquareNbX + l_u8SquareNbY);
    }
  }
